...
const specialties = ["全部", ...Array.from(new Set(therapists.map(t => t.specialty)))];
const priceRanges = [
...
const MassageTherapistGrid = ({ category }: { category?: CompanionCategory | null }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return therapists.filter(t => {
      ...
      if (activeTab === "recommended" && !t.isRecommended) return false;
  ...
  }, [search, city, specialty, priceIdx, category, activeTab]);
  ...
  return (
    <section className="py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mx-auto mb-4 flex w-full max-w-3xl items-center justify-center gap-2 rounded-full bg-muted/60 p-1 shadow-sm">
          {tabs.map((tab) => (
            <button
              key={tab.key}
              type="button"
              ...
              className={`flex-1 rounded-full px-4 py-2 text-sm font-semibold transition duration-200 ${
                activeTab === tab.key
                  ...
                  : "bg-slate-700/20 text-slate-300 hover:bg-slate-700/50 hover:text-white"
              }`}
            >
              {tab.label}
            </button>
              ...
              return (
                <div key={t.id} className="gradient-card border border-border rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-300 group hover:shadow-glow cursor-pointer" onClick={() => router.push(`/therapist/${t.id}`)}>
                  <div className="flex h-full min-h-[130px]">
                    <div className="w-40 shrink-0 overflow-hidden bg-muted">
                        ...
                        {images.length > 1 && (
                          <>
                            <button onClick={(e) => { e.stopPropagation(); setImageSlide(p => ({ ...p, [t.id]: (imgIdx - 1 + images.length) % images.length })); }}
                              className="absolute left-2 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full bg-card/70 backdrop-blur-sm flex items-center justify-center text-muted-foreground hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                              <ChevronLeft className="w-4 h-4" />
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); setImageSlide(p => ({ ...p, [t.id]: (imgIdx + 1) % images.length })); }}
                              className="absolute right-2 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full bg-card/70 backdrop-blur-sm flex items-center justify-center text-muted-foreground hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="flex flex-1 flex-col justify-between p-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-secondary text-sm font-medium mb-3">
                          ...
                          <span className="text-xs text-muted-foreground">| {t.experience}年经验</span>
                        </div>
                          ...
                          {t.services.map((s, i) => (
                            <button key={s.name}
                              ...
                              className={`px-2 py-1 rounded-full text-[10px] font-medium transition-all ${
                                svcIdx === i ? "bg-slate-900 text-white" : "bg-muted/50 text-muted-foreground hover:bg-muted/80 hover:text-foreground"
                              }`}>
                              {s.name}
                            </button>
                          ))}
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          <div className="text-lg font-bold text-secondary">¥{currentService.price}</div>
                          <div className="text-[11px] text-muted-foreground">/{currentService.duration}</div>
                        </div>
...
export default MassageTherapistGrid;
