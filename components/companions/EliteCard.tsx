  ...
  return (
    <div className={`min-w-[160px] h-[220px] rounded-2xl overflow-hidden relative shadow-sm group cursor-pointer ${className}`}>
      ...
      <div className="absolute top-2 left-2 bg-black/30 backdrop-blur-md px-2 py-0.5 rounded-full flex items-center gap-1">
        ...
        <span className="text-white text-xs font-medium leading-none">在线</span>
      </div>
      <div className="absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t from-black/80 to-transparent p-3 flex flex-col justify-end">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1">
            {tags.map(tag => (
              <span key={tag} className="bg-white/20 text-white/70 text-xs px-1.5 py-0.5 rounded leading-none">
                {tag}
              </span>
            ))}
          </div>
          <span className="text-white/50 text-xs leading-none">/小时</span>
        </div>
      </div>
    </div>
  );
